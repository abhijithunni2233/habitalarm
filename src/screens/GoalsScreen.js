...
export default function GoalsScreen() {
  ...
  const addGoal = async() => {
    ...
    const g={id:`goal_${Date.now()}`,title:title.trim(),target:parseInt(target)||30,current:0,reward:reward.trim(),emoji,createdAt:new Date().toISOString()};
  ...
  return (
    <ScrollView style={{flex:1,backgroundColor:COLORS.bg}} contentContainerStyle={{paddingBottom:60}} showsVerticalScrollIndicator={false}>
      <LinearGradient colors={['#FF6B9D','#FF8C42']} style={s.header} start={{x:0,y:0}} end={{x:1,y:1}}>
        ...
        <Text style={s.headerSub}>Set goals, attach rewards</Text>
      ...
      {showAdd&&(
        <View style={s.addCard}>
          ...
          <TextInput style={s.input} value={title} onChangeText={setTitle} placeholder="Goal name" placeholderTextColor={COLORS.textMuted}/>
          <TextInput style={s.input} value={target} onChangeText={setTarget} placeholder="Target count" placeholderTextColor={COLORS.textMuted} keyboardType="numeric"/>
          ...
          <TextInput style={s.input} value={reward} onChangeText={setReward} placeholder="Or type custom reward…" placeholderTextColor={COLORS.textMuted}/>
      ...
      <View style={{paddingHorizontal:SPACING.md}}>
        {goals.length===0&&!showAdd&&(
          <View style={s.empty}>
            ...
            <Text style={s.emptySub}>Set a goal and reward yourself</Text>
          </View>
        )}
        {goals.map(g=>{
...
const s = StyleSheet.create({
  ...
  addCard:{marginHorizontal:SPACING.md,marginBottom:SPACING.md,backgroundColor:COLORS.bgCard,borderRadius:RADIUS.xl,padding:SPACING.md,...SHADOW.md},
  ...
  input:{backgroundColor:COLORS.bgElevated,borderRadius:RADIUS.md,borderWidth:1.5,borderColor:COLORS.border,padding:SPACING.md,color:COLORS.text,fontSize:15,marginBottom:SPACING.sm,fontWeight:'500'},
  ...
  goalCard:{backgroundColor:COLORS.bgCard,borderRadius:RADIUS.xl,padding:SPACING.md,marginBottom:SPACING.sm,...SHADOW.sm,borderWidth:1.5,borderColor:COLORS.border},
