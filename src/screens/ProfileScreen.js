...
const BADGES = [
  ...
  {id:'streak7',e:'🔥',l:'Week Warrior',d:'7-day streak',ok:(h,l)=>h.some(x=>getStreakCount(l,x.id)>=7)},
  {id:'streak30',e:'⚡',l:'Monthly Master',d:'30-day streak',ok:(h,l)=>h.some(x=>getStreakCount(l,x.id)>=30)},
  {id:'five',e:'💪',l:'Full Stack',d:'5+ habits active',ok:(h)=>h.length>=5},
  {id:'perfect',e:'⭐',l:'Perfect Day',d:'Complete all habits today',ok:(h,l)=>{
  ...
  {id:'mood7',e:'😄',l:'Mood Tracker',d:'Log mood 7 days',ok:(_,__,___,m)=>{
    for(let i=0;i<7;i++){
...
export default function ProfileScreen() {
  ...
  const reset = () => Alert.alert('Reset All','Delete all habits and progress?',[
    ...
    {text:'Reset',style:'destructive',onPress:async()=>{
      await Promise.all([Storage.saveHabits([]),Storage.saveLogs({}),Storage.saveMoods({}),Storage.saveUser({xp:0,name:user.name})]);
      load();Haptics.notificationAsync(Haptics.NotificationFeedbackType.Warning);
    }},
  ]);
  ...
  return (
    <ScrollView style={{flex:1,backgroundColor:COLORS.bg}} contentContainerStyle={{paddingBottom:60}} showsVerticalScrollIndicator={false}>
      <LinearGradient colors={[COLORS.primary,'#4CC9F0']} style={s.hero} start={{x:0,y:0}} end={{x:1,y:1}}>
        ...
        {editName
          ? <TextInput style={s.nameInput} value={tmpName} onChangeText={setTmpName} autoFocus onBlur={saveName} onSubmitEditing={saveName} maxLength={20}/>
      ...
      <View style={s.statsRow}>
        {[
          ...
          <View key={i} style={[s.statCard,{borderTopColor:x.color,borderTopWidth:4}]}>
            <Text style={[s.statVal,{color:x.color}]}>{x.val}</Text>
...
const s = StyleSheet.create({
  hero:{alignItems:'center',paddingTop:60,paddingBottom:SPACING.xl,borderBottomLeftRadius:RADIUS.xxl,borderBottomRightRadius:RADIUS.xxl,marginBottom:SPACING.md},
