...
import { View, Text, StyleSheet, TouchableOpacity, ScrollView, TextInput, Alert, Animated, Dimensions, Platform, Modal, PanResponder } from 'react-native';
...
function getDateStr(dateKey){if(!dateKey)return'';const[y,m,d]=dateKey.split('-');const date=new Date(y,m-1,d);return`${DAYS[date.getDay()]}, ${d} ${MONTHS[date.getMonth()]} ${y}`;}
...
function generateInsights(habits,logs,moods){
  ...
  if(moodVals.length>=3){const avg=(moodVals.reduce((a,b)=>a+b,0)/moodVals.length).toFixed(1);if(avg>=4)insights.push(`😄 Your average mood is ${avg}/5 — you're thriving!`);else if(avg>=3)insights.push(`😐 Your average mood is ${avg}/5 — decent. More habits usually improve mood.`);else insights.push(`😔 Your mood has been low (${avg}/5). Try adding a self-care or exercise habit.`);}
  const dayCounts=[0,0,0,0,0,0,0];
  Object.entries(logs).forEach(([key,dayLogs])=>{const d=new Date(key);const completed=Object.values(dayLogs).filter(Boolean).length;dayCounts[d.getDay()]+=completed;});
...
function OnboardingScreen({onDone}){
  ...
  const next=()=>{
    ...
    Animated.timing(slideAnim,{toValue:-width,duration:300,useNativeDriver:true}).start(()=>{
      slideAnim.setValue(width);setPage(p=>p+1);
        ...
        <TouchableOpacity activeOpacity={1} style={rm.sheet}>
          ...
          <View style={rm.body}>
            ...
            <View style={rm.emojiRow}>
              {REMARK_EMOJIS.map(e=>(<TouchableOpacity key={e} onPress={()=>{Haptics.selectionAsync();setEmoji(e);}} style={[rm.emojiBtn,emoji===e&&{backgroundColor:color+'22',borderColor:color,borderWidth:2}]}><Text style={{fontSize:26}}>{e}</Text></TouchableOpacity>))}
            </View>
...
function QuotesCard(){
  ...
  useEffect(()=>{
    const t=setInterval(()=>{
      Animated.parallel([Animated.timing(tx,{toValue:-30,duration:350,useNativeDriver:true}),Animated.timing(op,{toValue:0,duration:350,useNativeDriver:true})]).start(()=>{
        tx.setValue(30);setIdx(i=>(i+1)%QUOTES.length);
        Animated.parallel([Animated.spring(tx,{toValue:0,useNativeDriver:true,tension:120,friction:8}),Animated.timing(op,{toValue:1,duration:300,useNativeDriver:true})]).start();
      });
    },5000);
      ...
      <View style={{flexDirection:'row',gap:8,paddingVertical:4}}>
        {dates.map(d=>(
          <TouchableOpacity key={d.key} onPress={()=>onSelectDate(d.key)}
            style={[{width:52,paddingVertical:10,borderRadius:14,alignItems:'center',backgroundColor:C.card,borderWidth:1.5,borderColor:C.border},selectedDate===d.key&&{backgroundColor:C.primary,borderColor:C.primary},d.isToday&&selectedDate!==d.key&&{borderColor:C.primary}]}>
...
function HomeScreen({habits,logs,moods,user,remarks,setUser,setLogs,setMoods,setRemarks,setHabits,onAddHabit,onHabitDetail}){
  ...
  const toggle=async(h,forceNotDone=false)=>{
    ...
    if(forceNotDone){nl[selectedDate][h.id]=false;}else{nl[selectedDate][h.id]=!was;}
    const completing=!was&&!forceNotDone;
    const newXp=Math.max(0,user.xp+(completing?10:forceNotDone&&was?-10:was?-10:10));
    const nu={...user,xp:newXp};
    setLogs(nl);setUser(nu);
    await Store.set('logs',nl);await Store.set('user',nu);
    if(completing){
      playTick();
      const newDone=active.filter(hh=>nl[selectedDate]?.[hh.id]).length;
  ...
  const saveRemark=async(remark)=>{
    ...
    const nr={...remarks,[key]:remark};setRemarks(nr);await Store.set('remarks',nr);
  };
    ...
    <ScrollView style={{flex:1,backgroundColor:C.bg}} showsVerticalScrollIndicator={false} contentContainerStyle={{paddingBottom:100}}>
      <View style={st.header}>
        ...
        <TouchableOpacity onPress={onAddHabit} style={st.fab}>
          <LinearGradient colors={[C.primary,C.primaryLight]} style={st.fabGrad}><Text style={st.fabTxt}>＋</Text></LinearGradient>
        </TouchableOpacity>
      </View>
      <DateSelector selectedDate={selectedDate} onSelectDate={setSelectedDate}/>
      {selectedDate!==today&&(
        <View style={{marginHorizontal:16,marginBottom:12,backgroundColor:'#FF8C42'+'22',borderRadius:12,padding:10,borderWidth:1.5,borderColor:'#FF8C42'}}>
      ...
      <View style={{paddingHorizontal:16,marginBottom:16}}>
        <View style={st.progressCard}>
          <View style={st.ringWrap}>
            <View style={[st.ring,{borderColor:pct===100?C.success:C.primary}]}>
              <Text style={[st.ringPct,{color:pct===100?C.success:C.primary}]}>{pct}%</Text>
      ...
      <View style={{paddingHorizontal:16,marginBottom:16}}>
        ...
        <View style={{flexDirection:'row',gap:8,marginTop:8}}>
          {[{v:1,e:'😞',c:C.mood1},{v:2,e:'😕',c:C.mood2},{v:3,e:'😐',c:C.mood3},{v:4,e:'🙂',c:C.mood4},{v:5,e:'😄',c:C.mood5}].map(m=>(
            <TouchableOpacity key={m.v} onPress={()=>setMood(m.v)} style={[st.moodBtn,todayMood===m.v&&{backgroundColor:m.c+'22',borderColor:m.c}]}>
      ...
      {habits.map((h,i)=>{
        const isCompleted=!!logs[selectedDate]?.[h.id];
        ...
        return(
          <View key={h.id} style={{paddingHorizontal:16,marginBottom:10}}>
            <View style={{position:'relative'}}>
              ...
              <Animated.View {...panResponder.panHandlers} style={[st.habitCard,{backgroundColor:bg},{transform:[{translateX:swipeX},{scale:scaleAnim}]}]}>
                ...
                <TouchableOpacity style={{flex:1,marginLeft:10}} onPress={()=>onHabitDetail(h)}>
                  ...
                  <View style={{flexDirection:'row',flexWrap:'wrap',gap:6,marginTop:4}}>
                    {streak>0&&<Text style={st.habitStreak}>🔥 {streak}d</Text>}
    ...
    <View style={{flex:1,backgroundColor:C.bg}}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{padding:16,paddingBottom:60}}>
        ...
        <View style={st.inputWrap}>
          ...
          <TextInput style={st.input} value={name} onChangeText={setName} placeholder="e.g. Morning Run" placeholderTextColor={C.textMuted} maxLength={40} autoFocus={!existing}/>
        </View>
        <Text style={[st.label,{marginTop:20}]}>ICON</Text>
        <View style={{flexDirection:'row',flexWrap:'wrap',gap:6}}>
          {ICONS.map(ic=>(<TouchableOpacity key={ic} onPress={()=>{Haptics.selectionAsync();setIcon(ic);}} style={[st.iconBtn,icon===ic&&{backgroundColor:color+'22',borderColor:color,borderWidth:2}]}><Text style={{fontSize:22}}>{ic}</Text></TouchableOpacity>))}
        </View>
        <Text style={[st.label,{marginTop:20}]}>COLOR</Text>
        <View style={{flexDirection:'row',flexWrap:'wrap',gap:10}}>
          {C.palette.map(c=>(<TouchableOpacity key={c} onPress={()=>{Haptics.selectionAsync();setColor(c);}} style={[{width:36,height:36,borderRadius:18,backgroundColor:c,alignItems:'center',justifyContent:'center'},color===c&&{borderWidth:3,borderColor:'#fff'}]}>{color===c&&<Text style={{color:'#fff',fontWeight:'900'}}>✓</Text>}</TouchableOpacity>))}
        </View>
        ...
        <View style={{flexDirection:'row',justifyContent:'space-between',alignItems:'center',marginTop:20,marginBottom:8}}>
          ...
          <TouchableOpacity onPress={()=>{setEditIdx(null);setPickerDate(new Date());setShowPicker(true);}} style={[st.addAlarmBtn,{backgroundColor:color}]}>
        ...
        <View style={{flexDirection:'row',gap:6}}>
          {DAYS_SHORT.map((d,i)=>(<TouchableOpacity key={i} onPress={()=>toggleRest(i)} style={[st.dayBtn,restDays.includes(i)&&{backgroundColor:C.textMuted+'22',borderColor:C.textMuted}]}><Text style={[{fontSize:12,fontWeight:'800',color:C.text},restDays.includes(i)&&{color:C.textSub}]}>{d}</Text></TouchableOpacity>))}
        </View>
...
function MonthlyCalendarScreen({habits,logs,onBack}){
  ...
  const getDayStatus=(day)=>{
    const key=getDayKey(day);const dayLogs=logs[key]||{};const total=habits.length;
    if(total===0)return'none';const done=habits.filter(h=>dayLogs[h.id]).length;
    if(done===0)return'none';if(done===total)return'perfect';return'partial';
  };
      ...
      <ScrollView contentContainerStyle={{padding:16,paddingBottom:60}}>
        <View style={{flexDirection:'row',gap:16,marginBottom:16,justifyContent:'center'}}>
          {[{c:C.success,l:'Perfect'},{c:C.gold,l:'Partial'},{c:C.section,l:'None'}].map((x,i)=>(
            <View key={i} style={{flexDirection:'row',alignItems:'center',gap:6}}>
              <View style={{width:14,height:14,borderRadius:4,backgroundColor:x.c}}/><Text style={{fontSize:11,color:C.textSub,fontWeight:'600'}}>{x.l}</Text>
            </View>
          ))}
        </View>
        ...
        <View style={{flexDirection:'row',flexWrap:'wrap'}}>
          {cells.map((day,i)=>{
            ...
            const key=getDayKey(day);const status=getDayStatus(day);const isToday=key===today;
        ...
        <View style={{marginTop:20,backgroundColor:C.card,borderRadius:20,padding:16}}>
          ...
          {(()=>{
            ...
            for(let d=1;d<=daysInMonth;d++){const s=getDayStatus(d);const isFuture=new Date(year,month,d)>new Date();if(!isFuture){if(s==='perfect')perfect++;else if(s==='partial')partial++;else missed++;}}
            return(
...
function StatsScreen({habits,logs,moods,onMonthly}){
  ...
  const totalMinutes=habits.reduce((acc,h)=>{const done=Object.values(logs).filter(d=>d[h.id]).length;return acc+(done*(h.duration||0));},0);
    ...
    <ScrollView style={{flex:1,backgroundColor:C.bg}} showsVerticalScrollIndicator={false} contentContainerStyle={{paddingBottom:80}}>
      ...
      <View style={{paddingHorizontal:16}}>
        ...
        {habits.map(h=>{
          ...
          const totalHabitMin=Object.values(logs).filter(d=>d[h.id]).length*(h.duration||0);
          return(
            <View key={h.id} style={st.breakdownCard}>
              <View style={{width:44,height:44,borderRadius:14,backgroundColor:h.color||C.primary,alignItems:'center',justifyContent:'center'}}><Text style={{fontSize:20}}>{h.icon}</Text></View>
              <View style={{flex:1,marginLeft:12}}>
                <View style={{flexDirection:'row',justifyContent:'space-between'}}><Text style={{fontSize:14,fontWeight:'700',color:C.text,flex:1}}>{h.name}</Text><Text style={{fontSize:12,color:C.gold,fontWeight:'700'}}>🔥 {streak}</Text></View>
                <View style={{height:7,backgroundColor:C.section,borderRadius:99,overflow:'hidden',marginVertical:6}}><View style={{height:'100%',width:`${r7}%`,backgroundColor:h.color||C.primary,borderRadius:99}}/></View>
...
function ProfileScreen({user,setUser,habits,logs,moods,setHabits,setLogs,setMoods}){
  ...
  const BADGES=[
    ...
    {e:'⭐',l:'Perfect Day',d:'All habits done today',ok:()=>{const k=getTodayKey(),dl=logs[k]||{};return habits.length>0&&habits.every(h=>dl[h.id]);}},
      ...
      <View style={{marginHorizontal:16,marginBottom:16,backgroundColor:C.card,borderRadius:20,overflow:'hidden'}}>
        <TouchableOpacity onPress={()=>setShowInsights(v=>!v)} style={{padding:16}}>
          <LinearGradient colors={[C.primary,C.primaryLight]} style={{borderRadius:12,padding:14,flexDirection:'row',alignItems:'center',justifyContent:'space-between'}}>
            <View><Text style={{fontSize:15,fontWeight:'800',color:'#fff'}}>🧠 AI Habit Insights</Text><Text style={{fontSize:12,color:'rgba(255,255,255,0.80)',marginTop:2}}>Tap to see your habit personality</Text></View>
...
function HabitDetailScreen({habit,logs,remarks,onBack,onEdit,onDelete}){
  ...
  const days=[];
  for(let i=34;i>=0;i--){const d=new Date();d.setDate(d.getDate()-i);const key=`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`;const remark=remarks[`${key}_${habit.id}`];days.push({key,date:d.getDate(),done:!!(logs[key]&&logs[key][habit.id]),isToday:i===0,remark});}
  return(
    <ScrollView style={{flex:1,backgroundColor:C.bg}} showsVerticalScrollIndicator={false} contentContainerStyle={{paddingBottom:60}}>
      ...
      <View style={{flexDirection:'row',gap:10,paddingHorizontal:16,marginBottom:16}}>
        {[{v:`${r7}%`,l:'7-day rate'},{v:`${r30}%`,l:'30-day rate'},{v:streak,l:'Streak'}].map((x,i)=>(<View key={i} style={[st.statCard,{borderTopColor:color,borderTopWidth:4}]}><Text style={{fontSize:18,fontWeight:'900',color}}>{x.v}</Text><Text style={{fontSize:10,color:C.textSub,marginTop:4,fontWeight:'600',textAlign:'center'}}>{x.l}</Text></View>))}
      </View>
      ...
      <View style={{paddingHorizontal:16,marginBottom:16}}>
        ...
        <View style={{flexDirection:'row',flexWrap:'wrap',gap:5}}>
          {days.map((d,i)=>(<View key={i} style={[{width:38,height:46,borderRadius:8,alignItems:'center',justifyContent:'center'},d.done&&{backgroundColor:color},!d.done&&{backgroundColor:C.section},d.isToday&&{borderWidth:2,borderColor:color}]}><Text style={{fontSize:10,fontWeight:'700',color:d.done?'#fff':C.textSub}}>{d.date}</Text>{d.done&&<Text style={{fontSize:9,color:'#fff',fontWeight:'900'}}>✓</Text>}{d.remark&&<Text style={{fontSize:9}}>{d.remark.emoji}</Text>}</View>))}
        </View>
      </View>
...
export default function App(){
  ...
  useEffect(()=>{
    setupNotifications();
    Promise.all([Store.get('habits',[]),Store.get('logs',{}),Store.get('moods',{}),Store.get('user',{xp:0,name:'Champion'}),Store.get('goals',[]),Store.get('remarks',{}),Store.get('onboarded',false)]).then(([h,l,m,u,g,r,onboarded])=>{
