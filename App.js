...
function getStreak(logs,id){
  ...
  for(let i=0;i<365;i++){
    ...
    if(logs[key]&&logs[key][id])streak++;else if(i>0)break;
  }
  return streak;
}
    ...
    <ScrollView style={{flex:1,backgroundColor:C.bg}} showsVerticalScrollIndicator={false} contentContainerStyle={{paddingBottom:100}}>
      ...
      <View style={{paddingHorizontal:16,marginBottom:16}}>
        <View style={st.progressCard}>
          ...
          <View style={{flex:1}}>
            <Text style={st.progressTitle}>Today's Progress</Text>
        ...
        <View style={{flexDirection:'row',gap:8,marginTop:8}}>
          {[{v:1,e:'😞',c:C.mood1},{v:2,e:'😕',c:C.mood2},{v:3,e:'😐',c:C.mood3},{v:4,e:'🙂',c:C.mood4},{v:5,e:'😄',c:C.mood5}].map(m=>(
            <TouchableOpacity key={m.v} onPress={()=>setMood(m.v)}
              style={[st.moodBtn,todayMood===m.v&&{backgroundColor:m.c+'22',borderColor:m.c}]}>
  ...
  return(
    <View style={{flex:1,backgroundColor:C.bg}}>
      <LinearGradient colors={[C.primary,C.primaryLight]} style={st.modalHeader}>
        ...
        <TouchableOpacity onPress={save} disabled={saving} style={st.saveHdrBtn}>
          <Text style={st.saveHdrTxt}>{saving?'…':'Save'}</Text>
        </TouchableOpacity>
      </LinearGradient>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{padding:16,paddingBottom:60}}>
        ...
        <View style={{flexDirection:'row',justifyContent:'space-between',alignItems:'center',marginTop:20,marginBottom:8}}>
          ...
          <TouchableOpacity onPress={()=>{setEditIdx(null);setPickerDate(new Date());setShowPicker(true);}}
            style={[st.addAlarmBtn,{backgroundColor:color}]}>
        ...
        <View style={{flexDirection:'row',gap:6}}>
          {DAYS_SHORT.map((d,i)=>(
            <TouchableOpacity key={i} onPress={()=>toggleRest(i)}
              style={[st.dayBtn,restDays.includes(i)&&{backgroundColor:C.textMuted+'22',borderColor:C.textMuted}]}>
              <Text style={[{fontSize:12,fontWeight:'800',color:C.text},restDays.includes(i)&&{color:C.textSub}]}>{d}</Text>
            </TouchableOpacity>
          ))}
        </View>
        ...
        <View style={[st.habitCard,{backgroundColor:color}]}>
          ...
          <View style={{flex:1,marginLeft:10}}>
            ...
            {alarms.length>0&&<Text style={{fontSize:11,color:'rgba(255,255,255,0.80)',marginTop:3}}>⏰ {alarms.map(fmtAlarm).join(' · ')}</Text>}
          </View>
          <View style={st.check}/>
        </View>
      </ScrollView>
                      ...
                      return(
                        <View key={dk.key} style={{width:36,alignItems:'center'}}>
                          <View style={[{width:24,height:24,borderRadius:6,alignItems:'center',justifyContent:'center'},
                            ...
                            dk.isToday&&!done&&{borderWidth:1.5,borderColor:C.primary+'50'}]}>
            ...
            <View key={h.id} style={st.breakdownCard}>
              ...
              <View style={{flex:1,marginLeft:12}}>
                <View style={{flexDirection:'row',justifyContent:'space-between'}}>
                  ...
                  <Text style={{fontSize:12,color:C.gold,fontWeight:'700'}}>🔥 {streak}</Text>
                </View>
                ...
                <Text style={{fontSize:11,color:C.textSub}}>7d: {r7}% · 30d: {r30}%</Text>
              </View>
            </View>
          );
        })}
...
function GoalsScreen({goals,setGoals}){
  ...
  const add=async()=>{
    ...
    const u=[...goals,g];setGoals(u);await Store.set('goals',u);
  ...
  const del=async(id)=>{
    Alert.alert('Delete','Remove this goal?',[
      ...
      {text:'Delete',style:'destructive',onPress:async()=>{
        const u=goals.filter(g=>g.id!==id);setGoals(u);await Store.set('goals',u);
      }},
    ]);
  };
          ...
          <View style={{flexDirection:'row',flexWrap:'wrap',gap:8,marginBottom:12}}>
            {GEMOJIS.map(e=>(
              <TouchableOpacity key={e} onPress={()=>setEmoji(e)}
                style={[{width:44,height:44,borderRadius:10,backgroundColor:C.section,alignItems:'center',justifyContent:'center'},
                  emoji===e&&{backgroundColor:C.primaryPale,borderWidth:2,borderColor:C.primary}]}>
          ...
          <View style={{flexDirection:'row',flexWrap:'wrap',gap:6,marginBottom:8}}>
            {RCHIPS.map(r=>(
              <TouchableOpacity key={r.l} onPress={()=>setReward(r.l)}
                style={[{paddingHorizontal:10,paddingVertical:6,backgroundColor:C.section,borderRadius:99},
                  reward===r.l&&{backgroundColor:C.goldPale,borderWidth:1.5,borderColor:C.gold}]}>
...
function ProfileScreen({user,setUser,habits,logs,moods,setHabits,setLogs,setMoods}){
  ...
  const saveName=async()=>{
    ...
    const u={...user,name:tmpName.trim()};setUser(u);await Store.set('user',u);setEditName(false);
  };
